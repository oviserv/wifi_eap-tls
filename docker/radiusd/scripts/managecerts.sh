...
# Set datecode for naming CA and Radius server
...
set_value_in_section server commonName "\"${SERVERNAME}\"" "${SRVCONFIG}"
...
# Store the Radius server name as this information is needed to configure
# some clients
echo "${SERVERNAME}" > "${RADIUSSERVERFILE}"
...
# Copy CA file and file with Radius server name to provision dir
